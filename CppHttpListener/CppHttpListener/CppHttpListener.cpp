// CppHttpListener.cpp : Defines the entry point for the console application.
//
...
SendHttpPostResponse(
	IN HANDLE        hReqQueue,
	IN PHTTP_REQUEST pRequest, ULONG*
);
...
)
{
	...
	for (;;)
	{
		...
		result = HttpReceiveHttpRequest(
			hReqQueue,          // Req Queue
			requestId,          // Req ID
			HTTP_RECEIVE_REQUEST_FLAG_FLUSH_BODY,                  // Flags
		...
		if (NO_ERROR == result)
		{
			...
			{
			case HttpVerbGET:
				wprintf(L"Got a GET request for %ws. Received %d bytes\n", pRequest->CookedUrl.pFullUrl, bytesRead);
			...
			case HttpVerbPOST:
			{
				...
				result = SendHttpPostResponse(hReqQueue, pRequest, &TotalBytesRead);
				...
				if (last_report == 0 || (end - last_report) >= CLOCKS_PER_SEC * 1)
				{
					last_report = end;

					double fps = (double)totalFramesHandled * CLOCKS_PER_SEC / (double)(end - start);
			...
			default:
				wprintf(L"Got a unknown request for %ws. Received %d bytes \n", pRequest->CookedUrl.pFullUrl, bytesRead);
	...
	if (pRequest->Flags & HTTP_REQUEST_FLAG_MORE_ENTITY_BODY_EXISTS)
	{
		...
		do
		{
			...
			case ERROR_HANDLE_EOF:
				...
				*TotalBytesRead2 = TotalBytesRead;
